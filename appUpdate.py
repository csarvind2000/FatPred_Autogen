...
def generate_pdf_report(inference_results, recommendations, file_name='BodyFat_Report.pdf'):
    pdf = FPDF()
    ...
    pdf.cell(200, 10, txt="Body Fat Prediction & Recommendation Report", ln=True, align='C')
    ...
    for index, row in inference_results.iterrows():
        pdf.cell(200, 10, txt=f"Sample {index+1}: Actual BodyFat = {row['Actual BodyFat']:.2f}, Predicted BodyFat = {row['Predicted BodyFat']:.2f}", ln=True)
    ...
    pdf.set_font('Arial', '', 10)
    for index, rec in enumerate(recommendations.split('\n')):
...
Ensure that plots are displayed in Streamlit using st.pyplot().
Provide only the code, without additional explanations or comments.
"""
...
class AgentController:
    ...
    def perform_inference(self):
        ...
        X_test = st.session_state.get('X_test')
        y_test = st.session_state.get('y_test')
        scaler = st.session_state.get('scaler')  # Get the scaler used in training
        ...
        if best_model is not None and X_test is not None and y_test is not None:
            ...
            if sample_indices:
                selected_samples = X_test.iloc[sample_indices]
                true_values = y_test.iloc[sample_indices].reset_index(drop=True)

                # Scale the selected samples using the same scaler
                selected_samples_scaled = scaler.transform(selected_samples)

                # Generate predictions for the selected samples
                predictions = best_model.predict(selected_samples_scaled)
                predictions_series = pd.Series(predictions, name='Predicted BodyFat')

                # Create a results DataFrame
                results_df = selected_samples.copy()
                results_df['Actual BodyFat'] = true_values.values
                results_df['Predicted BodyFat'] = predictions_series.values

                st.subheader("Inference Results:")
                st.write(results_df)

                # Store in session state for further use (e.g., report generation)
                ...
                if st.button("Download Report"):
                    with st.spinner("Generating PDF report..."):
                        report_file = generate_pdf_report(results_df, recommendations)
                        st.success(f"Report generated successfully! [Download]({report_file})")
            else:
